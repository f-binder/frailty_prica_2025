# Import and exploration - new dataset
# Using prior code as template
library(tidyverse)

thepath <- "data/raw/Frailty and CAR T - Frailty Data -10Apr2026 - clean.xlsx"
...
long %>%
  ...
  arrange(timepoint_n, variable) %>% 
  rename_with(~str_remove(., fixed("numeric."))) %>%
  View() # to see if it maches reported summary measures (done for prior version, ok)


# CFS score at baseline
str_subset(tolower(names(base)), "cfs")

  # label of other studies to plot
